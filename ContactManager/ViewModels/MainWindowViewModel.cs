...
/* 
  ...
  =========================================
  Этот файл является ViewModel главного окна в паттерне MVVM. Он представляет собой "мозг" приложения.
  Здесь сосредоточена вся бизнес-логика, и этот код ничего не знает о XAML-разметке интерфейса.

  ЧТО ОН ДЕЛАЕТ И ЗАЧЕМ НУЖЕН КРАТКО:
  1. IDbContextFactory — внедряется через конструктор (DI). Нужна для безопасного создания короткоживущих 
     контекстов БД в асинхронных потоках, чтобы интерфейс приложения никогда не зависал.
  2. ObservableCollection — используется для списков Contacts и Departments. В отличие от List, она автоматически 
     уведомляет визуальную таблицу DataGrid на экране о добавлении или удалении элементов.
  3. SearchText — свойство поиска. Как только пользователь вводит символ на экране, сеттер мгновенно запускает LoadDataAsync().
  4. SelectedContact — отслеживает, на кого кликнули в таблице. Если строка выбрана, её данные копируются в форму.
  5. Валидация — свойства IsFormValid, ShowFullNameError проверяют поля на пустоту и блокируют кнопку сохранения, выводя ошибку.
  6. CurrentWindowIndex — управляет переключением 3 вкладок (0-просмотр, 1-добавление, 2-изменение) через свойство IsVisible в XAML.
  7. Soft Delete (Мягкое удаление) — метод ConfirmDeleteContactAsync() не вызывает физический Remove. Он меняет флаг IsActive 
     на false, сохраняет это в SQLite, и запись просто скрывается от пользователя благодаря фильтру .Where(c => c.IsActive).
  =========================================
*/
